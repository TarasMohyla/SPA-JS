...
export class Card extends DivComponent {
    ...
    render() {
        ...
        <div class="card__image">
            <img src="https://covers.openlibrary.org/b/olid/${this.cardState.cover_edition_key}-M.jpg" alt="Photo" />
        </div>

        <div class="card__info">
            <div class="card__tag">${this.cardState.subject ? this.cardState.subject[0] : 'Not specified'}</div>
            <div class="card__name">${this.cardState.title}</div>
            <div class="card__author">${this.cardState.author_name ? this.cardState.author_name[0] : 'Anonym'}</div>
            <div class="card__footer">
                <button class="button_add ${existInFavorites ? 'button__active' : ''}">
                    ${existInFavorites 
                        ?  '<img src="/static/favorite-active.svg" alt="Favorite" />' : '<img src="/static/favorite_add.svg" alt="Favorite" />'
                    }
                </button>
            </div>
        </div>
